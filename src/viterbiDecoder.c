...
int viterbiDecoderHard(viterbiHardState_t* state, uint8_t* codedSegments, uint8_t* uncoded, int segmentsIn, bool last){
    ...
    for(int i = 0; i<segmentsIn; i++){
        ...
        METRIC_TYPE pathMetrics[NUM_STATES][POW2(k)];
        ...
        for(int dstState = 0; dstState<NUM_STATES; dstState++){
            ...
            for(int edgeIn = 0; edgeIn<POW2(k); edgeIn++){
                ...
                METRIC_TYPE srcMetric = state->nodeMetrics[srcNodeIdx];
            ...
            for(int edgeIn = 0; edgeIn<POW2(k); edgeIn++){
                pathMetrics[dstState][edgeIn] = srcNodeMetrics[edgeIn] + edgeMetric[edgeIn];
            }
        }
        ...
        for(int dstState = 0; dstState<NUM_STATES; dstState++){
            ...
            int minPathEdgeInIdx = argminPathMetrics(&(pathMetrics[dstState]));
            int minPathSrcNodeIdx = dstState/POW2(k) + minPathEdgeInIdx*POW2((S-1)*k);
            newMetrics[dstState] = pathMetrics[dstState][minPathEdgeInIdx];
            ...
            TRACEBACK_TYPE newTB = state->traceBack[minPathSrcNodeIdx];
            newTB = newTB << k;
            newTB |= edgeOut;
        ...
        if(state->iteration >= TRACEBACK_LEN){
            ...
            int minNodeIdx = argminNodeMetrics(&newMetrics);
            TRACEBACK_TYPE nodeTB = newTraceback[minNodeIdx];
        ...
        //Copy new states back in
        for(int node = 0; node<NUM_STATES; node++){
            state->nodeMetrics[node] = newMetrics[node];
            state->traceBack[node] = newTraceback[node];
        }
    }
    ...
    if(last){
        ...
        TRACEBACK_TYPE tb = state->traceBack[0];
